...
const PostItContainerCalendar = ({ date, groupId }) => {
    ...
    const [endpoint, setEndpoint] = useState(`${GET_POST_IT_BY_DATE.replace("${groupId}", groupId)}${date}`);
    const { authToken } = useAuth();
    const { data, error, fetch: fetchData } = useFetch(
        endpoint,
        {
          method: "GET",
          headers: {
    ...
    useEffect(() => {
        setEndpoint(
            `${GET_POST_IT_BY_DATE.replace("${groupId}", groupId)}${date}`
        );
    }, [date]);
